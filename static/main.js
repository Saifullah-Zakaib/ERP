...
function initializeCharts() {
    ...
    // Inventory Chart - Use real data from Django or fallback to demo data
    const inventoryCtx = document.getElementById('inventoryChart');
    console.log('Inventory canvas found:', inventoryCtx !== null);
    
    if (inventoryCtx) {
        let inventoryData = {
            labels: ['BALL-001', 'BALL-002', 'SHOE-001', 'JERSEY-001', 'BAT-001'],
            currentStock: [450, 85, 320, 180, 65],
            reorderLevel: [100, 100, 50, 50, 80]
        };
        
        // Use real data if available and not empty
        if (window.dashboardData && 
            window.dashboardData.inventory && 
            window.dashboardData.inventory.labels.length > 0) {
            inventoryData = window.dashboardData.inventory;
            console.log('Using real inventory data:', inventoryData);
        } else {
            ...
            new Chart(inventoryCtx, {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: 'Current Stock',
                            data: inventoryData.currentStock,
                            backgroundColor: chartColors.primary,
                            borderRadius: 6
                        },
                        {
                            label: 'Reorder Level',
                            data: inventoryData.reorderLevel,
    ...
    // Supplier Chart - Use real data from Django or fallback to demo data
    const supplierCtx = document.getElementById('supplierChart');
    console.log('Supplier canvas found:', supplierCtx !== null);
    
    if (supplierCtx) {
        let supplierData = {
            labels: ['Global Sports', 'Premium Fabrics', 'TechPack'],
            percentages: [95, 88, 92]
        };
        
        // Use real data if available and not empty
        if (window.dashboardData && 
            window.dashboardData.supplier && 
            window.dashboardData.supplier.labels.length > 0) {
            supplierData = window.dashboardData.supplier;
            console.log('Using real supplier data:', supplierData);
        } else {
        ...
        try {
            ...
            console.log('Supplier chart created successfully');
        } catch (error) {
            console.error('Error creating supplier chart:', error);
        }
    }

    // Forecast Chart - Dynamic Data
    const forecastCtx = document.getElementById('forecastChart');
    if (forecastCtx) {
        let forecastLabels = ['Oct 2025', 'Nov 2025', 'Dec 2025', 'Jan 2026', 'Feb 2026'];
        let forecastData = [850, 920, 1045, 980, 1020];
        
        // Use real data if available
        if (window.dashboardData && 
            window.dashboardData.forecast && 
            window.dashboardData.forecast.labels.length > 0) {
            forecastLabels = window.dashboardData.forecast.labels;
            forecastData = window.dashboardData.forecast.demand;
            console.log('Using real forecast data:', forecastLabels, forecastData);
        } else {
...
// Progress range display update(Production Dash) - DISABLED OLD CODE
// The production page now uses inline JavaScript with API calls
